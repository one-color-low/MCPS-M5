...
void initBLE() {
  ...
  //pServer and pService

  // Create the BLE Server
  pServer = BLEDevice::createServer(); // <- !! サーバー化している。M5が親機側なので
  pServer->setCallbacks(new MyServerCallbacks()); // <- connect用コールバック

  // Create the BLE Service
  BLEService *pService = pServer->createService(SERVICE_UUID);

  // Create a BLE Characteristic
  pNotifyCharacteristic = pService->createCharacteristic( // notify(M5->iPhone)用の設定
  ...
  BLECharacteristic * pRxCharacteristic = pService->createCharacteristic( // write(iPhone->M5)用の設定
